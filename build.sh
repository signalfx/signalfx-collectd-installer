...
for x in signalfx-metadata/10-signalfx.conf collectd-write_http/10-write_http-plugin.conf collectd-aggregation/10-aggregation-cpu.conf collectd/collectd.conf.tmpl collectd-match_regex/filtering.conf; do
	curl -sSL "https://raw.githubusercontent.com/signalfx/integrations/master/${x}" > `basename $x`
done

tar -cvzf install-files.tgz *.conf collectd.conf.tmpl get_aws_unique_id get_all_auth_tokens.py
